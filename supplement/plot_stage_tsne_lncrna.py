"""
t-SNE on TCGA primary tumor samples with AJCC stage (lncRNA expression matrix).
...
def main() -> None:
    ap = argparse.ArgumentParser(description="4D t-SNE on stage-sample lncRNA matrix → four PNGs.")
    ...
    base = "fig_tsne_stage_lncrna_samples"
    ...
    def save_pair(dim_i: int, dim_j: int, suffix: str) -> Path:
        ...
        if suffix.endswith("cancer_type"):
            _scatter_panel(
                ...
                dim_j,
                cancer,
                f"t-SNE (dims {dim_i + 1} vs {dim_j + 1}) — cancer type",
                "cancer_type",
                stage_mode=False,
            )
        else:
            _scatter_panel(
                ...
                dim_j,
                stage,
                f"t-SNE (dims {dim_i + 1} vs {dim_j + 1}) — AJCC stage",
                "stage",
                stage_mode=True,
            )
