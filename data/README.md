...
**Supplement sensitivity / combo grid / TTN SB overview** (`generate_netmhc_supplement.py` without `--skip-sensitivity`): needs the **same merged TSVs** as the canonical NetMHC bundle; no extra cohort files. **Rebuild merged TSVs** from wide XLS + IEDB CSVs: `python rebuild_netmhc_merged_tsvs.py` (see `--help`; IEDB CSVs often live only on your machine under a sibling `data/netmhc/`).
