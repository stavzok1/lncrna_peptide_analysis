...
Run from repository root::
    ...
    python scripts/regenerate_manuscript_netmhc_figures.py --clean --purge-repo-figures-netmhc \\
...
TTN_IEDB_CSV = ROOT / "data" / "netmhc" / "ttn_as1_iedb_companion_synthetic.csv"
TTN_IEDB_PARENT_ID = "108065|TTN-AS1|synthetic"


def resolve_freq_file() -> Path:
    for rel in (
        Path("data/netmhc/figures/fig5a_epitopes_vs_allele_frequency_ic50_sb.csv"),
        Path("data/netmhc/figures/epitopes_vs_allele_frequency_ic50_sb.csv"),
    ):
        p = ROOT / rel
        if p.is_file():
            return p
    raise FileNotFoundError(
        ...
        f"  {ROOT / 'data/netmhc/figures/fig5a_epitopes_vs_allele_frequency_ic50_sb.csv'}\n"
        f"  {ROOT / 'data/netmhc/figures/epitopes_vs_allele_frequency_ic50_sb.csv'}"
    )
...
def main() -> None:
    ...
    if not args.skip_wide:
        ...
        for metric in METRICS:
            ...
            run(
                [
                    py,
                    str(ROOT / "plot_netmhc_epitopes_vs_hla_frequency.py"),
                    "--freq-file",
                    str(freq),
                    "--y-metric",
                    metric,
            ...
            run(
                [
                    py,
                    str(ROOT / "plot_figure5b_epitope_sharing_across_alleles.py"),
                    "--histogram-metric",
                    metric,
                    "--out-png",
            ...
            run(
                [
                    py,
                    str(ROOT / "plot_figure5de_epitopes_per_allele.py"),
                    "--count-metric",
                    metric,
                    "--out-png",
        ...
        for sb in SB_MODES:
            for metric in METRICS:
                ...
                run(
                    [
                        py,
                        str(ROOT / "scripts" / "plot_fig5abc_netmhc_sb_triple.py"),
                        "--allele-freq-csv",
                        str(freq),
                        "--sb-mode",
                        sb,
                ...
                run(
                    [
                        py,
                        str(ROOT / "scripts" / "plot_fig5de_merged_iedb_sb_per_allele.py"),
                        "--sb-mode",
                        sb,
                        "--out-dir",
                        str(d_de),
    ...
    if not args.skip_fig6:
        ...
        run(
            [
                py,
                str(ROOT / "plot_figure6_ttn_as1_allele_coverage.py"),
                "--gating",
                "netmhc",
                "--split-panels",
                "--also-write-unique",
    ...
    if not args.skip_sensitivity:
        for sb in SB_MODES:
            for metric in METRICS:
                ...
                run(
                    [
                        py,
                        str(ROOT / "scripts" / "netmhc_sb_sensitivity_robustness.py"),
                        "--sb-mode",
                        sb,
                        "--plot-metric",
                        metric,
        ...
        run(
            [
                py,
                str(ROOT / "plot_figure6_ttn_as1_sb_sensitivity.py"),
                "--out-dir",
                str(s6),
            ],
            dry_run=dry,
    ...
    if args.with_combination_grid:
        ...
        run(
            [
                py,
                str(ROOT / "scripts" / "plot_fig5_netmhc_sb_combination_grid.py"),
                "--out-dir",
                str(cg),
            ],
            dry_run=dry,
